...
/**
 * Apply compact formatting: strip leading indents and collapse empty lines
 * so the text becomes a continuous flowing stream without blank gaps.
 */
export function applyCompact(rawText: string): string {
  let text = rawText.replace(/\r\n/g, '\n');
  // Strip leading/trailing whitespace from each line (including Chinese full-width space)
  text = text.replace(/^[ \t\u3000]+|[ \t\u3000]+$/gm, '');
  // Collapse 2+ consecutive newlines into a single paragraph marker \x00
  text = text.replace(/\n{2,}/g, '\x00');
  // Remove all remaining single newlines (hard line-wraps inside paragraphs)
  text = text.replace(/\n/g, '');
